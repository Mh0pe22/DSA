...
public class DivideNodesIntoMaximumNumberOfGroups {
    ...
    public static boolean isBipartite(Map<Integer , List<Integer>> adj , int curr , int[] colors , int currColor){
    ...
    public static int bfs(Map<Integer , List<Integer>> adj , int currNode , int n){
    ...
    public static int getMaxFromEachComp(Map<Integer , List<Integer>> adj , int curr , boolean[] visited , int[] levels){
        int maxGroup = levels[curr];
        visited[curr] = true;

        for(int ngbr : adj.get(curr)){
            if(!visited[ngbr]){
                maxGroup = Math.max(maxGroup , getMaxFromEachComp(adj , ngbr , visited , levels));
            }
        }
    ...
    public static int magnificentSets(int n , int[][] edges){
        ...
        // sum the max group size for each component
